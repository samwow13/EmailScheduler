...
namespace EmailScheduler.Services
{
    ...
    public class EmailService : IEmailService
    {
        ...
        public async Task SendAccountActivationEmailsAsync()
        {
            ...
            _logger.LogInformation("Starting email sending process at {Time} with {Count} recipients", 
            ...
            try
            {
                ...
                _logger.LogInformation("Connected to SMTP server {Server}:{Port}", 
                ...
                foreach (var recipient in _emailSettings.Recipients)
                {
                    try
                    {
                        ...
                        successCount++;
                        _logger.LogInformation("✓ Email successfully sent to {Recipient} at {Time}", 
                            recipient, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                    }
                    catch (SmtpException smtpEx)
                    {
                        failureCount++;
                        _logger.LogError(smtpEx, "✗ SMTP error sending email to {Recipient}: {ErrorMessage} (Status: {Status})", 
                            recipient, smtpEx.Message, smtpEx.StatusCode);
                    }
                    catch (Exception ex)
                    {
                        failureCount++;
                        _logger.LogError(ex, "✗ Failed to send email to {Recipient}: {ErrorMessage}", 
                            recipient, ex.Message);
                    }
                }

                _lastSendTime = DateTime.Now;
                _logger.LogInformation("Email sending process completed. Success: {SuccessCount}, Failures: {FailureCount}", 
                    successCount, failureCount);
            }
            catch (SmtpException smtpEx)
            {
                _logger.LogError(smtpEx, "SMTP error connecting to server {Server}:{Port}: {ErrorMessage} (Status: {Status})", 
                    _emailSettings.SmtpServer, _emailSettings.SmtpPort, smtpEx.Message, smtpEx.StatusCode);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error connecting to SMTP server {Server}:{Port}: {ErrorMessage}", 
                    _emailSettings.SmtpServer, _emailSettings.SmtpPort, ex.Message);
            }
        }
        ...
        public bool ShouldSendEmails()
        {
            ...
            if (_emailSettings.UseTestSchedule)
            {
                ...
                if (shouldSend)
                {
                    _logger.LogInformation("Test schedule triggered: {Minutes} minutes elapsed since last send (threshold: {Threshold})", 
                        Math.Round(minutesSinceLastSend, 2), _emailSettings.TestIntervalMinutes);
                }
            ...
            var isWeeklyScheduleTime = now.DayOfWeek == (DayOfWeek)_emailSettings.WeeklyScheduleDay && 
            ...
            {
                _logger.LogInformation("Weekly schedule triggered: Day {Day}, Hour {Hour}", 
                    now.DayOfWeek, now.Hour);
            }
        ...
        private SmtpClient CreateSmtpClient()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(_emailSettings.Username) && !string.IsNullOrEmpty(_emailSettings.Password))
                {
                    ...
                    _logger.LogDebug("Using credentials for user {Username}", _emailSettings.Username);
                }
                else
                {
                    _logger.LogWarning("No SMTP credentials provided - anonymous authentication will be used");
                }
        ...
        private MailMessage CreateEmailMessage(string recipient)
        {
            try
            {
                ...
                var message = new MailMessage
                {
                    ...
                    Body = $"This is an automated email to keep your account active. No action is required.\n\nSent at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}",
                    IsBodyHtml = false
                };
                
                message.To.Add(recipient);
                _logger.LogDebug("Created email message to {Recipient} with subject '{Subject}'", 
            ...
            {
                _logger.LogError(ex, "Invalid email format. Sender: '{Sender}', Recipient: '{Recipient}'", 
                    _emailSettings.SenderEmail, recipient);
                throw;
            }
            catch (Exception ex)
            {
